 ...
 */
static int pro_instrument_enable_full_report(struct hid_device *hdev)
{
					...
					struct hid_input *hidinput)
{
	...
	if (sc->quirks & NAVIGATION_CONTROLLER_USB) {
		...
		ret = pro_instrument_enable_full_report(hdev);
		if (ret < 0) {
...
static const struct hid_device_id sony_devices[] = {
	 ...
	 */
	{ HID_USB_DEVICE(USB_VENDOR_ID_HARMONIX, USB_DEVICE_ID_HARMONIX_WII_RB1_GUITAR_DONGLE),
	...
	{ HID_USB_DEVICE(USB_VENDOR_ID_SONY_RHYTHM, USB_DEVICE_ID_SONY_PS3_RB3_MUSTANG_GUITAR),
		.driver_data = PS_INSTRUMENT | RB3_PS3_PRO_INSTRUMENT },
